...
A session can be driven from another OpenCode process (the user's interactive window shares session storage with the headless server the orchestrator uses). Those turns never emit `session.idle` on the orchestrator's event stream, so the task may stay `RUNNING` with `session_status=busy` even after the transcript shows a completed assistant turn. Durable transcript evidence is authoritative: when the transcript ends on a completed assistant message with `step-finish` and no input is pending, `collect_result` reconciles the task to `COMPLETED` and collects. A wedged turn (trailing assistant message incomplete, busy forever) is not fabricated as complete — after the stall threshold the task goes `STALLED`, and `reply_and_wait` with `kind=continue` then accepts the task to send a continuation prompt into the same session.

If the same OpenCode session receives new activity after the task was recorded as `COMPLETED` or `ABORTED` — whether from the user in OpenCode or from you — `task_status` may project live `RUNNING`, `INPUT_REQUIRED`, or a finished new turn as `COMPLETED/COLLECTING` without mutating the record. Reacquire it through `resume_wait` while busy or while waiting for external activity (it blocks and adopts the session on new live activity), an exact still-pending permission/question reply while input is pending, `reply_and_wait` with `kind=continue` and `payload.reacquire=true` when you must inject the next message yourself, or `collect_result` after the new turn completed. Those lease-owned operations preserve the task/session/worktree and invalidate stale review state. An earlier abort is marked `SUPERSEDED` only after new live activity is detected or an explicit reacquire; old heartbeats, stale tool records, and an idle session without a newer message do not reopen it.
...
| `ABORTED` | Report the partial state. To continue the same task, either wait for or detect new live session activity (`resume_wait` blocks and adopts it), or when you should drive the next step yourself, call `reply_and_wait` with `kind=continue` and `payload.reacquire=true` in the original session. Never create a replacement task or session for the same approved work. |
| `STALLED` | Inspect diagnostics and surface pending input. First check `task_status`: if the transcript finished (`last_turn_finished`), call `collect_result` to reconcile it. If the turn wedged (busy with no progress), call `reply_and_wait` with `kind=continue` to nudge the same session once it is idle. Do not abort or create a replacement session. |
...
When the user explicitly asks to continue the same approved task after a local interruption, use `reply_and_wait` with `kind=continue` only if the persisted phase is `PAUSED` or `STALLED`, OpenCode is idle, and there are no pending permissions, questions, or tools. The tool rechecks all gates, acquires the existing task's WaitLease, opens SSE before sending, reuses the original model/effort, and records a unique continuation marker for no-resend recovery. If OpenCode is busy, use `resume_wait` instead. For a `COMPLETED` or `ABORTED` task that must be driven further in the same session, pass `payload.reacquire=true` as explicit confirmation; the tool supersedes the terminal record, invalidates stale review state, and injects the continuation into the original session. Never call the internal OpenCode client directly.
...
Single-tool cancellation is not a production capability in 2.1.6: the disposable experiment records `production_supported=false`. The public surface remains exactly eight tools; `kind=continue` reuses `reply_and_wait`, and recovery otherwise uses a permission reply, `resume_wait`, or explicit `abort_task`.
